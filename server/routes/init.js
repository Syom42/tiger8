...
import { eq, or, isNull, asc, desc, inArray } from 'drizzle-orm';
...
// Safe query runner — returns fallback on error so one bad query doesn't kill everything.
...
app.get('/init', requireAuth, async (c) => {
  const uid = Number(c.get('uid')); // coerce to number — old JWTs may have string uid

  // Run all queries in parallel, each isolated so one failure doesn't kill others.
  const [
    profileRows,
    exerciseRows,
    workoutRows,
  ...
  // Assemble workout exercises + sets (scoped to this user's workouts only)
  let workoutsWithExercises = workoutRows.map(w => ({ ...w, exercises: [] }));
  if (workoutRows.length > 0) {
    const wIds = workoutRows.map(w => w.id);
    const [exRows, setRows] = await Promise.all([
      safe('workout_exercises', () =>
        db.select().from(workoutExercises)
          .where(inArray(workoutExercises.workoutId, wIds))
          .orderBy(asc(workoutExercises.sortOrder)),
        []),
      safe('workout_sets', () => {
        // Subquery: only sets for exercises in this user's workouts
        return db.select().from(workoutSets)
          .where(inArray(workoutSets.workoutExerciseId,
  ...
  let plansWithExercises = planRows.map(p => ({ ...p, exercises: [] }));
  if (planRows.length > 0) {
    const pExRows = await safe('plan_exercises', () =>
  ...
  const suppData = Array.isArray(suppRows) ? suppRows : (suppRows?.rows ?? []);

  console.log(`[init] uid=${uid} plans=${plansWithExercises.length} workouts=${workoutsWithExercises.length}`);
